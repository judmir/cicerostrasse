...
app.whenReady().then(async () => {
  ...
  await run(`$('[data-inspiration-edit]').click();field('#inspiration-note','Edited note survives reload.');$('.inspiration-composer').requestSubmit();await wait(()=>!$('.inspiration-composer'),'Edited');$('#plan-toggle').click();if($('#plan-navigation').hidden)throw new Error('Plan expansion');($('.plan-room-label[data-room="bad"]')||$('[data-plan-room="bad"]')).click();await wait(()=>location.hash==='#/room/bad'&&$('#room-title').textContent==='Bad','Plan room navigation');if(!$('#plan-navigation').hidden)throw new Error('Plan did not collapse');$('#inspiration-link').click();await wait(()=>$('.inspiration-empty')&&!$('#inspiration-gallery').hidden,'Room isolation');$('#plan-toggle').click();($('.plan-room-label[data-room="kuche"]')||$('[data-plan-room="kuche"]')).click();await wait(()=>location.hash==='#/room/kuche'&&$('#room-title').textContent==='Küche','Return to sources');$('#inspiration-link').click();await wait(()=>!$('#inspiration-gallery').hidden&&document.querySelectorAll('[data-inspiration-item]').length===3,'Return to room');`);
